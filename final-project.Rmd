...
#My error was in Line 289, so it appeared to skip this part!
```
...
For example, say we want to use the NHANES data to calculate the mean for age of first use for each drug and depression score. Since there is quite a bit of missing values in this data set, so we will need to add arguments to the mean function to ignore the missing data. 
...
#It appears pivot_longer can't be found and I couldn't find it in my help documentation. I googled it and found its part of tidyverse, but re-installing it, Rstudio, and R still didn't work for me. Sorry for this issue on my end!
...
Things you did well:
1. This is so well put together! Your tutorial explanations are already so thorough! I'm very impressed! 
2. I appreciated how function 3 built upon function 2 to show that you can adapt the function to include a second grouping variable. The functions are great and the flow/explanation works very well for the tutorial. Also, its cool to see you use the same names for the variables in each function, its like building your own package of functions :)
3. I really appreciated how you showed errors and possible other ways to write the same code. It was great for a tutorial to show alternatives and ways that you might mess up and why.

Things I learned:
1. That pivot_longer is available and an alternative to gather! Bummer that I can't get it to work on my computer, but still cool to see in use!!
2. I'd never seen rpois used before (or don't remember it), so cool to see in use as well and as another option than rnorm.

Areas for improvement:
1. For your plot function, it might be nice to show people how to change the axis number sizes within the function to make it easier to read!
