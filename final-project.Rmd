...
*TN: Great way of organizing the data! but I wonder if you could put all the data together in the same place and have a variable that identifies old from more recent data? * 
...
# TN: good point about finding out how to keep code and error!! I think that you could just cite the code with r markdown and mention what the error was
...
*TN: I loved how you explained map by steps, first showing how you owuld do it with one vector and how to reach to the idea of using map, excellent! * 
...
# TN: I like that you used a simple verctor/list to explain double to make it simple and clear to understand! 
...
# TN: It would have probably been nice to do map with a list of lists too, to see how powerful it can be - we can even loop through multiple data frames!! 
...
m2_db <- map2_dbl(obs, wts, ~weighted.mean(.x, .y)) #I just saved it because I don't know how useful it is to see it printed 
...
## TN: I am running into an error when I run the above: Error in pivot_longer(., cols = age_cocaine:age_marijuana, names_to = c("dis",  : could not find function "pivot_longer"
...
```
# TN this looks like a great way of explaining how mutate and nest can be useful together! I actually learned a lot by looking at the last piece of code, very useful! Sadly I was having trouble running the chunk because I kept running on an error 
...
by_year_drug <- d_long %>% 
  group_by(year, drug) %>% #Count is not in the data frame so I am removing it
  nest() %>% 
  mutate(count = map(data, ~mutate(.x, count = n()))) #this doesnt run #now it runs! I think "count" was in the group by by mistake 
```

# TN: this was suuper useful! and I was able to run it, I like how you use mutate with nest to create a list of useful data frames! 
...
# TN: great way of explaining fucntions through steps, it made it clear and easy to understand! 
# TN: it could also the process for creatign a function of starting from  inside -> out to make the process more clear
...
fun4_plot(recent_drug) #TN this doesnt run 
```

# TN: These are all great functions!!! And I love the step - by - step explanations. It would probably be nic to add warning and error messages when creating them, plus it would be a total plus for the intructional plan!! 
