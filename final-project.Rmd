...
Each chunk was labelled descriptively and examples were thorough and easy to follow. 
...
#this is a really clear explanation of purr::map
...
#Once again, really clear and straightforward examples. 
...
map_chr(v, round) #one thing you could show here is that can you specfiy how many decimal places to round to. 
```
...
d #for the tutorial it might be helpful to name the dataset something descriptive like age_drug_use
```
...
#I like that you showed and described two different ways of doing the same thing. 
...
#for the purpose of the tutorial it might be helpful to use head() so that 1,000 numbers don't print out! Not necessary, but just a minor suggestion! 
```
...
# I get an error message that says pivot_longer could not be found. I found online that it's a part of tidyverse but it's still not running. 
```{r nest-mutate1}
dat <- recent_drug %>% 
  select(-1:-2, -age_stim) %>% 
...
#I like that you explain the pipe operator but this may be better somewhere else because the pipe has been used throughout the code so far. 
...
fun2_mean1group <- function(data, cat_var, num_var){
  ...
  data %>% #if (cat_var2 < 17)
    ...
    summarize(mean = mean(!!num_var, na.rm = TRUE),
              ...
              max = max(!!num_var, na.rm = TRUE)) # i put these on separate lines for clarity
}

fun2_mean1group(recent_drug, mental_health, age_marijuana)
#when I run this there is a row for NAs, it might be helpful to know why that is. Or it might be better to remove it all together. 
...
#I see it says this doesn't work, but I think the issue is coming from the read.csv line. Sorry I'm not more helpful. 
